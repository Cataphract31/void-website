  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2">
      <div className="display text-[15px] font-bold tracking-[0.16em]">
        CRITICAL<span className="text-[var(--color-amber)]">MASS</span>
      </div>
      <div className="label">#{snap.roundId}</div>
      <div className="ml-auto flex items-center gap-3">
        <div className="text-right">
          <div className="label">wallet</div>
          <div className="tnum text-[13px] font-semibold text-[var(--color-amber)]">
    ...
    <div className="flex items-center gap-3 px-3">
      ...
      <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--color-panel2)]">
        <div
          ...
          style={{
            width: `${live ? pct : 0}%`,
            background: `linear-gradient(90deg, var(--color-cyan), var(--color-amber) 55%, var(--color-danger))`,
          }}
        />
      </div>
...
export function ActionBar({
  ...
  const bg =
    tone === "go"
      ? "bg-[var(--color-profit)] text-[#04140b]"
      : tone === "cash"
        ? "bg-[var(--color-amber)] text-[#1a1206]"
